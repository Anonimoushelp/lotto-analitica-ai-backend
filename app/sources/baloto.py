...
class BalotoAdapter(OfficialSourceAdapter):
    ...
    def parse(self, payload: str) -> NormalizedDraw:
        pattern = re.compile(
            ...
            r"(?P<n1>\\d{2})\\s*-\\s*(?P<n2>\\d{2})\\s*-\\s*(?P<n3>\\d{2})\\s*-\\s*(?P<n4>\\d{2})\\s*-\\s*(?P<n5>\\d{2})\\s*-\\s*(?P<bonus>\\d{2})",
        ...
        draw_date = datetime(int(year), months[month], int(day)).date()
