...
class MappingSourceAdapter:
    ...
    def normalize(self, payload: Mapping[str, Any]) -> RawDrawRecord:
        lottery_code = str(
            self._get(payload, "lottery_code", self.spec.lottery_code)
        )
        ...
        main_numbers = self._coerce_numbers(
            self._get(payload, "main_numbers"),
            "main_numbers",
        )
    ...
    def _coerce_numbers(value: Any, field: str) -> list[int]:
        ...
        if any(number < 1 or number > MAX_SOURCE_NUMBER for number in numbers):
            raise SourceNormalizationError(
                f"{field} numbers must be between 1 and {MAX_SOURCE_NUMBER}"
            )
