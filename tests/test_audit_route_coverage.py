...
EXPECTED_AUDITED_MUTATIONS = {
    "auth.py": {("register", "create", "bootstrap_admin"), ("update_user", "update_credentials", "user")},
    "lotteries.py": {
...
def test_audit_route_coverage_does_not_include_read_or_login_handlers():
    for filename in EXPECTED_AUDITED_MUTATIONS:
        tree = ast.parse((ROUTES_DIR / filename).read_text(encoding="utf-8"))
        functions = {
            node.name: node
            for node in tree.body
        ...
        audited_functions = {entry[0] for entry in EXPECTED_AUDITED_MUTATIONS[filename]}
