...
def test_phase_384_failed_delete_recovery_then_cross_provider_reimport_is_atomic():
    ...
    try:
        ...
        assert stats(db, lottery.id, "baloto-colombia") == {5101: 1, 5102: 1, 5103: 1}
        assert stats(db, lottery.id, "revancha-colombia") == {5201: 1, 5202: 1, 5203: 1}
        ...
        assert stats(db, lottery.id, "baloto-colombia") == {}
        assert stats(db, lottery.id, "revancha-colombia") == {5201: 1, 5202: 1, 5203: 1}
        assert stats(db, lottery.id, "miloto-colombia") == {5301: 1, 5302: 1, 5303: 1}
...
def test_phase_384_failed_cross_lottery_reassignment_then_reimport_keeps_four_scopes_isolated():
    ...
    try:
        ...
        target = seed_draw(db, first.id, "38410", date(2027, 1, 10), [5401, 5402, 5403], "baloto-colombia")
        conflict = seed_draw(db, second.id, "38411", date(2027, 1, 11), [5501, 5502, 5503], "baloto-colombia")
        seed_draw(db, second.id, "38412", date(2027, 1, 12), [5601, 5602, 5603], "revancha-colombia")
        seed_draw(db, first.id, "38413", date(2027, 1, 13), [5701, 5702, 5703], "miloto-colombia")

        original_commit = db.commit
        for cycle in range(3):
            db.commit = lambda: (_ for _ in ()).throw(IntegrityError("forced", {}, Exception()))
            with pytest.raises(HTTPException) as exc:
        ...
        assert stats(db, first.id, "baloto-colombia") == {5401: 1, 5402: 1, 5403: 1}
        assert stats(db, first.id, "miloto-colombia") == {5701: 1, 5702: 1, 5703: 1}
        assert stats(db, second.id, "baloto-colombia") == {5501: 1, 5502: 1, 5503: 1}
        assert stats(db, second.id, "revancha-colombia") == {5601: 1, 5602: 1, 5603: 1}

        LotteryDrawService.delete_draw(db, target.id)
        LotteryDrawService.create_draw(
            db=db, lottery_id=first.id, draw_number="38414", draw_date=date(2027, 1, 14),
            main_numbers=[5901, 5902, 5903], source="baloto-colombia",
        )
        assert stats(db, first.id, "baloto-colombia") == {5901: 1, 5902: 1, 5903: 1}
        assert stats(db, first.id, "miloto-colombia") == {5701: 1, 5702: 1, 5703: 1}
        assert stats(db, second.id, "baloto-colombia") == {5501: 1, 5502: 1, 5503: 1}
        assert stats(db, second.id, "revancha-colombia") == {5601: 1, 5602: 1, 5603: 1}
        assert db.get(LotteryDraw, target.id) is None
...
def test_phase_384_stale_session_after_failed_recovery_cannot_restore_cross_scope_state():
    ...
    try:
        ...
        target = seed_draw(db, lottery.id, "38420", date(2027, 1, 20), [6001, 6002, 6003], "baloto-colombia")
        survivor = seed_draw(db, lottery.id, "38421", date(2027, 1, 21), [6101, 6102, 6103], "revancha-colombia")
        ...
        with pytest.raises(HTTPException) as exc:
            LotteryDrawService.update_draw(
                ...
                update_data={
                    "draw_number": "38499", "draw_date": date(2027, 1, 31),
        ...
        assert stats(db, lottery.id, "baloto-colombia") == {6001: 1, 6002: 1, 6003: 1}
        assert stats(db, lottery.id, "revancha-colombia") == {6101: 1, 6102: 1, 6103: 1}
        ...
        assert stats(stale, lottery.id, "baloto-colombia") == {6001: 1, 6002: 1, 6003: 1}
        assert db.get(LotteryDraw, survivor.id) is not None
    finally:
        stale.close()
        db.close()
