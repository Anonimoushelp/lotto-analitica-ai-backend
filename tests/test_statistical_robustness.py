...
def test_large_valid_number_values_remain_exact_in_frequency_and_pairs():
    result = StatisticalService.analyze(
        [make_draw(1, [1_000_000, 2_000_000, 2_000_001])]
    )
...
def test_none_main_numbers_are_excluded_without_affecting_valid_metrics():
    result = StatisticalService.analyze(
        [make_draw(1, None), make_draw(2, [3, 4]), make_draw(3, None)]
    )
    ...
    assert result["number_recency"] == {
        ...
        4: {"last_seen_draw": 1, "draws_since_seen": 0},
    }
