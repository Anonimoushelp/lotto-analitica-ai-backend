...
from app.scheduler.runner import SchedulerRunner, SchedulerStatus, classify_ingestion_error, evaluate_schedule
...
def test_scheduler_explicit_status_before_and_after_window():
    ...
    assert evaluate_schedule(schedule, datetime(2026, 9, 21, 12, 59, tzinfo=COLOMBIA_TZ)) == SchedulerStatus.NOT_PUBLISHED
    assert evaluate_schedule(schedule, datetime(2026, 9, 21, 13, 15, tzinfo=COLOMBIA_TZ)) == SchedulerStatus.SUCCESS
    assert evaluate_schedule(schedule, datetime(2026, 9, 21, 13, 31, tzinfo=COLOMBIA_TZ)) == SchedulerStatus.OUTSIDE_TOLERANCE


def test_scheduler_marks_non_scheduled_weekday_as_not_published():
    schedule = ScheduledDraw("TEST", "DIA", time(13, 0), weekdays=frozenset({0}))
    assert evaluate_schedule(schedule, datetime(2026, 9, 22, 13, 10, tzinfo=COLOMBIA_TZ)) == SchedulerStatus.NOT_PUBLISHED


def test_runner_reports_not_published_and_does_not_ingest():
    calls: list[tuple[str, str]] = []
    runner = SchedulerRunner(lambda code, draw: calls.append((code, draw)) or "persisted", schedules=(ScheduledDraw("TEST", "DIA", time(13, 0)),))
    attempts = runner.run_once(datetime(2026, 9, 21, 12, 59, tzinfo=COLOMBIA_TZ))
    assert attempts == [type(attempts[0])("TEST", "DIA", SchedulerStatus.NOT_PUBLISHED, "schedule not due")]
    assert calls == []


def test_runner_reports_outside_tolerance_and_does_not_ingest():
    calls: list[tuple[str, str]] = []
    runner = SchedulerRunner(lambda code, draw: calls.append((code, draw)) or "persisted", schedules=(ScheduledDraw("TEST", "DIA", time(13, 0), tolerance_minutes=30),))
    attempts = runner.run_once(datetime(2026, 9, 21, 14, 0, tzinfo=COLOMBIA_TZ))
...
def test_scheduler_isolates_same_lottery_different_draw_types():
    ...
    attempts = SchedulerRunner(ingest, schedules=schedules).run_once(datetime(2026, 9, 21, 13, 10, tzinfo=COLOMBIA_TZ))
    assert [attempt.status for attempt in attempts] == [SchedulerStatus.DUPLICATE, SchedulerStatus.SUCCESS]
...
def test_calendar_only_catalog_schedule_is_executable_on_official_weekday():
    ...
    risaralda = next(item for item in schedules if item.lottery_code == "LOTERIA_RISARALDA")
    assert risaralda.calendar_only is True
    calls: list[tuple[str, str]] = []
    attempts = SchedulerRunner(lambda code, draw: calls.append((code, draw)) or "controlled", schedules=(risaralda,)).run_once(
        datetime(2026, 9, 21, 10, 0, tzinfo=COLOMBIA_TZ)
    )
...
def test_catalog_scheduler_only_exposes_verified_ready_sources():
    bindings = build_catalog_scheduler_bindings()
    ready = {item.lottery_code for item in bindings if item.calendar_rule in ready_catalog_schedules(bindings)}
...
def test_holiday_schedule_reaches_runner():
    schedule = ScheduledDraw("TEST", "NOCHE", time(19, 25), frozenset({6}), 90, holiday_times=(time(19, 28),), skip_on_holiday=True)
    calls: list[tuple[str, str]] = []
    attempts = SchedulerRunner(lambda code, draw: calls.append((code, draw)) or "holiday", schedules=(schedule,)).run_once(
...
def test_dated_schedule_supports_super_astro_variable_times():
    schedule = DatedSchedule((DatedDrawTime(date(2026, 9, 21), "ASTRO_SOL", time(13, 0)), DatedDrawTime(date(2026, 9, 21), "ASTRO_LUNA", time(22, 0))))
    assert schedule.is_due(datetime(2026, 9, 21, 13, 30, tzinfo=COLOMBIA_TZ), "ASTRO_SOL")
    assert schedule.is_due(datetime(2026, 9, 21, 22, 45, tzinfo=COLOMBIA_TZ), "ASTRO_LUNA")
    assert not schedule.is_due(datetime(2026, 9, 21, 12, 59, tzinfo=COLOMBIA_TZ), "ASTRO_SOL")


def test_dated_schedule_rejects_duplicate_calendar_entries():
    schedule = DatedSchedule((DatedDrawTime(date(2026, 9, 21), "ASTRO_SOL", time(13, 0)), DatedDrawTime(date(2026, 9, 21), "ASTRO_SOL", time(14, 0))))
...
def test_fixed_schedule_remains_outside_window():
    schedule = ScheduledDraw("TEST", "DIA", time(13, 0), tolerance_minutes=30)
    attempts = SchedulerRunner(lambda *_: "persisted", schedules=(schedule,)).run_once(datetime(2026, 9, 21, 14, 0, tzinfo=COLOMBIA_TZ))
    assert attempts[0].status == SchedulerStatus.OUTSIDE_TOLERANCE
