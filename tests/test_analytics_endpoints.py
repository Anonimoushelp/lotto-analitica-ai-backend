...
def test_statistical_overview_contract(monkeypatch):
    monkeypatch.setattr(
        StatisticalService,
        "overview",
        lambda db: {"module_status": "READY", "algorithms_count": 6, "draws_analyzed": 42},
    )
    response = client.get("/api/v1/statistics/overview")
    assert response.status_code == 200
    assert response.json() == {
        "module_status": "READY", "algorithms_count": 6, "draws_analyzed": 42
    }
...
def test_statistical_service_advanced_algorithms_are_deterministic():
    ...
    assert result["consecutive_numbers"] == {
        "draws_with_consecutive": 2, "total_consecutive_pairs": 6, "maximum_consecutive_pairs": 4
    }
...
def test_statistical_service_handles_empty_and_single_draw_boundaries():
    assert StatisticalService.analyze([]) == {
        ...
        "pair_frequency": {},
        "consecutive_numbers": {"draws_with_consecutive": 0, "total_consecutive_pairs": 0, "maximum_consecutive_pairs": 0},
    }
...
def test_statistical_service_stays_standby_without_draws():
    ...
    assert StatisticalService.overview(EmptyDb()) == {
        "module_status": "STANDBY", "algorithms_count": 0, "draws_analyzed": 0
    }
