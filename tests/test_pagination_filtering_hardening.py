...
def test_draw_list_default_limit_is_bounded():
    ...
    for index in range(105):
        seed_draw(lottery.id, f"PAGE-{index:03d}", (index % 28) + 1)
...
def test_draw_list_order_is_deterministic_with_id_tiebreaker():
    ...
    first = seed_draw(lottery.id, "PAGE-A", 10)
    second = seed_draw(lottery.id, "PAGE-B", 10)

    response = client.get(
        "/api/v1/draws",
        params={"lottery_id": lottery.id, "limit": 2},
        headers=auth_header(analyst),
    )
...
def test_draw_list_filter_and_limit_compose_without_cross_resource_leakage():
    ...
    for index in range(4):
        seed_draw(first.id, f"PAGE-3-{index}", 11 + index)
        seed_draw(second.id, f"PAGE-4-{index}", 21 + index)

    response = client.get(
        "/api/v1/draws",
        params={"lottery_id": first.id, "limit": 3},
        headers=auth_header(analyst),
    )
...
def test_draw_list_unknown_filter_is_stable_for_large_positive_ids():
    ...
    seed_draw(lottery.id, "PAGE-5-1", 25)

    response = client.get(
        "/api/v1/draws",
        params={"lottery_id": 9223372036854775807, "limit": 500},
        headers=auth_header(analyst),
    )
